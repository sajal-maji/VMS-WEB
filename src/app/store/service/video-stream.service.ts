...
class VideoneticsRTC extends EventTarget {
  ...
  constructor(base_url: any, videoElement: any) {
    ...
    this.#video.addEventListener('error', (ev: any) => {
      console.warn(ev);
      this.dispatchEvent(
        new CustomEvent('error', { detail: { error: new VideoneticsRTCError(ev.error?.message) } }),
      );
      // this.#ondisconnect()
    });
      ...
      if (typeof ev.data === 'string') {
        ...
        switch (type) {
          ...
          case 'error':
            ...
            this.#onmessage['webrtc'](msg);
            this.dispatchEvent(
              new CustomEvent('error', { detail: { error: new VideoneticsRTCError(msg.value) } }),
            );
            break;
          default:
            this.#onmessage['hls'](msg);
      ...
      sb.addEventListener('updateend', () => {
        ...
        try {
          ...
          if (bufLen > 0) {
            ...
            bufLen = 0;
            if (ms.sourceBuffers.length > 0 && ms.sourceBuffers[0] === sb) sb.appendBuffer(data);
            // Todo Monotosh
          ...
          } else if (ms.sourceBuffers.length > 0 && sb.buffered && sb.buffered.length) {
